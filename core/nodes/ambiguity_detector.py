...
class AmbiguityOutput(BaseModel):
    ...
    clarification_options: list[str] = Field(description="If ambiguous, 2-3 short options in French to clarify. If not, empty list.", default=[])
...
def ambiguity_node(state: CopilotState) -> CopilotState:
    ...
    If location is missing assume it's for the whole Montreal city. If timeframe is missing, assume it's for all available data. Only flag as ambiguous if the lack of this information would lead to a completely different answer or if the question is too vague to even attempt a database query.
